...
# Install cloudflare-ms {{{
COPY --chown=1000:1000 . /opt/fmc_repository/cloudflare-ms
RUN install_repo_deps.sh /opt/fmc_repository/cloudflare-ms/

# Cleanup repository {{{
RUN rm -rf /opt/fmc_repository/cloudflare-ms/{.git,docker,Dockerfile}
# }}}
# Build tarball {{{
